...
async def test_order_pickup(
    authenticated_ac: AsyncClient,
    product_id,
    st_code_add_basket,
    store_id,
    st_code_create_order,
    count_orders,
):
    """Тестирование добавление в корзину - создание заказа"""
    response_basket = await authenticated_ac.post(
        "/orders/add_to_basket", params={"product_id": product_id}
    )
    assert response_basket.status_code == st_code_add_basket
    response_create_order = await authenticated_ac.post(
        "/orders/create_order_pickup", json={"store_id": store_id}
    )
