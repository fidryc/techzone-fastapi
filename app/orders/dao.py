...
class BasketDao(BaseDao):
    ...
    async def add_product(self, product_id: int, user_id: int):
        ...
        try:
            ...
            logger.error(
                msg, extra={"product_id": product_id, "user_id": user_id}, exc_info=True
            )
            raise HTTPException(
    ...
    async def price_of_basket(self, user_id: int) -> float:
        ...
        try:
            ...
            logger.debug(
                "Basket price calculated", extra={"user_id": user_id, "price": price}
            )
            return price
        except SQLAlchemyError:
    ...
    async def basket_of_user(self, user_id: int) -> list[tuple[int, int]]:
        ...
        try:
            ...
            logger.debug(
                "Basket contents retrieved",
                extra={"user_id": user_id, "items_count": len(result)},
            )
            return result
    ...
    async def basket_of_user_with_quantity(self, user_id: int) -> list[tuple[int, int]]:
        ...
        try:
            result = (await self.session.execute(query, params)).fetchall()
            logger.debug(
                "Basket contents with quantity retrieved",
                extra={"user_id": user_id, "items_count": len(result)},
            )
            return result
        except SQLAlchemyError as e:
    ...
    async def delete_basket_of_user(self, user_id) -> list[int]:
        ...
        try:
            ...
            logger.debug(
                "User basket deleted",
                extra={"user_id": user_id, "deleted_count": len(deleted_ids)},
            )
            return deleted_ids
        except SQLAlchemyError:
...
class OrderDao(BaseDao):
    ...
    async def change_status(self, status: str, order_id: int):
        """Смена статуса заказ"""
        logger.debug(
            "Changing order status", extra={"order_id": order_id, "new_status": status}
        )
        query = text(
            """
            UPDATE orders
        ...
        try:
            ...
            logger.error(
                msg, extra={"status": status, "order_id": order_id}, exc_info=True
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def get_active_user_orders(self, user_id: int) -> list[int]:
        ...
        try:
            ...
            logger.debug(
                ...
                extra={"user_id": user_id, "orders_count": len(active_orders)},
            )
...
class OrderPickUpDetailsDao(BaseDao):
    ...
    async def add_and_return_id(self, **details):
        ...
        try:
            ...
            logger.debug(
                "Pickup details created successfully",
                extra={"order_pickup_detail_id": detail_id},
            )
            return detail_id
        except SQLAlchemyError as e:
...
class OrderDeliveryDetailDao(BaseDao):
    ...
    async def add_and_return_id(self, **details):
        ...
        query = (
            insert(self.model)
            .values(**details)
            .returning(OrderDeliveryDetail.order_delivery_detail_id)
        )
        try:
            res = await self.session.execute(query)
            detail_id = res.scalar()
            logger.debug(
                ...
                extra={"order_delivery_detail_id": detail_id},
            )
            return detail_id
        except SQLAlchemyError as e:
            msg = create_msg_db_error(
                "Cannot add_and_return_id for OrderDeliveryDetail"
            )
            logger.error(msg, extra={"details": details}, exc_info=True)
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
class PurchaseDao(BaseDao):
    ...
    async def add_products_of_order(self, basket_of_user, order_id):
        ...
        logger.debug(
            ...
            extra={"order_id": order_id, "basket_items": len(basket_of_user)},
        )
        data_for_insert = []
        ...
        try:
            ...
            logger.debug(
                ...
                extra={"order_id": order_id, "products_count": len(data_for_insert)},
            )
        except SQLAlchemyError as e:
