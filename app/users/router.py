...
from app.users.services import NotificationServiceFactory, RegisterService
...
async def register_with_email(
    ...
    await register_service.initiate_registration(
        request, response, user, notification_service
    )


@router.post(
    path="/register_with_number", summary="Инициализация регистрации по номеру телефона"
)
async def register_with_number(
    request: Request,
    response: Response,
    user: UserRegisterNumberSchema,
    redis_service: RedisServiceDep,
    user_dao: UserDaoDep,
    session: SessionDep,
):
...
@router.post(
    path="/verify_code", summary="Подверждение кода, чтобы успешно зарегестрироваться"
)
...
async def login_with_email(
    response: Response, user: UserAuthEmailSchema, user_service: UserServiceDep
):
    """
...
async def login_with_number(
    response: Response, user: UserAuthNumberSchema, user_service: UserServiceDep
):
    """
...
@router.post(path="/delete_user", summary="Удаление аккаунта")
async def delete_user(
    response: Response, user: CurrentUserDep, user_service: UserServiceDep
):
    """
