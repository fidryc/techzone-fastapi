...
class RefreshTokenBLService:
    ...
    async def processing_refresh_token(self, response: Response, refresh_token):
        ...
        try:
            bl_token = await self.refresh_token_bl_dao.get_refresh_token_bl(
                refresh_token["jti"]
            )
            if bl_token:
                logout_user(response)
                logger.warning(
                    "Refresh token in blacklist", extra={"jti": refresh_token["jti"]}
                )
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN, detail="Сессия заблокирована"
                )
            logger.debug(
                "Refresh token successfully processed",
                extra={"jti": refresh_token["jti"]},
            )
        except HTTPException:
            raise
        except Exception:
            logger.error(
                "Unexpected error in processing_refresh_token",
                extra={"jti": refresh_token.get("jti")},
                exc_info=True,
            )
            raise HTTPException(
                status_code=500, detail="Ошибка при проверке refresh token"
            )
...
class UserService:
    ...
    async def delete_user(self, user: UserSchema, response: Response):
        ...
        try:
            ...
            if active_orders:
                logger.info(
                    "Account deletion refused: User has active orders",
                    extra={
                ...
                raise HTTPException(
                    ...
                    detail="Пользователь имеет активные заказы",
                )
            ...
            logger.error(
                "Failed to delete user", extra={"user_id": user.user_id}, exc_info=True
            )
            await self.session.rollback()
            raise HTTPException(
    ...
    async def get_user_from_token(
        ...
        try:
            ...
            if not user_from_access_token:
                logger.warning(
                    "User not found from access token",
                    extra={
                        "email": token_payload.get("user_email"),
                        "number": token_payload.get("user_number"),
                    },
                )
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
                    detail="Неверный email или number",
                )

            validate_exp_token(token_payload)
            logger.debug(
                "User retrieved from access token",
                extra={"user_id": user_from_access_token.user_id},
            )
            ...
            try:
                ...
                await self.refresh_token_bl_service.processing_refresh_token(
                ...
                if not user_from_refresh_token:
                    logger.warning(
                        "User not found from refresh token",
                        extra={
                            "email": refresh_token.get("user_email"),
                            "number": refresh_token.get("user_number"),
                        },
                    )
                    raise HTTPException(
                        status_code=status.HTTP_401_UNAUTHORIZED,
                ...
                logger.info(
                    ...
                    extra={"user_id": user_from_refresh_token.user_id},
                )
                ...
                raise HTTPException(
                    status_code=500, detail="Ошибка при обновлении токена"
                )

    async def login_user(
        self, response: Response, user: UserAuthEmailSchema | UserAuthNumberSchema
    ):
        """
        Вход в аккаунт
        ...
        try:
            ...
            if not user_in_db:
                logger.warning(
                    ...
                    extra={"email": user.email, "number": user.number},
                )
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
                    detail="Неправильный номер телефона или почта",
                )

            if not check_pwd(user.password, user_in_db.hashed_password):
                logger.warning(
                    "Login failed: Incorrect password",
                    extra={"user_id": user_in_db.user_id},
                )
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
            ...
            logger.info(
                "User logged in successfully", extra={"user_id": user_in_db.user_id}
            )
        except HTTPException:
            raise
        except Exception:
    ...
    def logout_user(self, response: Response):
        try:
            ...
            raise HTTPException(
                status_code=500, detail="Ошибка при выходе с аккаунт"
            ) from e
...
class NotificationEmailService(NotificationService):
    ...
    def send_verification_code(self, code):
        ...
        try:
            ...
            logger.info(
                "Verification code sent to email", extra={"recipient": self.user.email}
            )
        except Exception:
            logger.error(
                "Failed to send verification code to email",
                extra={"recipient": self.user.email},
                exc_info=True,
            )
            raise HTTPException(
                status_code=500, detail="Ошибка при отправке кода на email"
            )
...
class NotificationNumberService(NotificationService):
    ...
    def send_verification_code(self, code):
        ...
        raise HTTPException(500, "Сервис для отправки кода еще не реализован")
...
class NotificationServiceFactory:
    ...
    def get_notification_service(self) -> NotificationService:
        ...
        if isinstance(self.user, UserRegisterEmailSchema):
            ...
            logger.error(
                "Unknown user type for notification service",
                extra={"user_type": type(self.user).__name__},
            )
...
class RegisterService:
    def __init__(
        self, user_dao: UserDao, redis_service: RedisService, session: AsyncSession
    ):
        self.user_dao = user_dao
        self.redis_service = redis_service
        self.session = session
    ...
    def _get_data_for_registration(self, user, code: int) -> UserAuthRedisSchema:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail="Ошибка при подготовке данных регистрации"
            )
    ...
    async def initiate_registration(
        ...
        request: Request,
        response: Response,
        user: UserRegisterEmailSchema | UserRegisterNumberSchema,
        ...
        try:
            ...
            logger.info(
                ...
                extra={"user_identifier": user_identifier, "ip": request.client.host},
            )
        except HTTPException:
            raise
        except Exception as e:
            ...
            raise HTTPException(
                status_code=500, detail="Ошибка при инициализации регистрации"
            ) from e
    ...
    async def confirm_and_register_user(
        ...
        try:
            ...
            if not data:
                logger.warning(
                    "Registration data not found in Redis", extra={"key": key}
                )
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
                    detail="Данные регистрации не найдены. Попробуйте зарегистрироваться снова",
                )
            ...
            if not self.redis_service.is_correct_attempt(data["attempt"]):
                logger.warning(
                    ...
                    extra={
                        "verify_key": verify_register_key,
                        "attempts": data["attempt"],
                    },
                )
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
                    detail="Попробуйте зарегистрироваться снова",
                )
            try:
                ...
                logger.warning(
                    ...
                    extra={"verify_key": verify_register_key},
                )
                raise HTTPException(
                    status_code=status.HTTP_409_CONFLICT, detail="Передан неверный код"
                )
            ...
            logger.error(
                "Failed to confirm and register user",
                extra={"verify_key": verify_register_key},
                exc_info=True,
            )
            await self.session.rollback()
            raise HTTPException(
                status_code=500, detail="Ошибка при завершении регистрации"
            ) from e
        finally:
