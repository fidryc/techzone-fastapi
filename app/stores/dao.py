...
class StoreQuantityInfoDao(BaseDao):
    ...
    async def get_quantity_of_product(self, store_id: int, product_id: int) -> int:
        quantity_in_store: list[StoreQuantityInfoSchema] = await self.find_by_filter(
    ...
    async def reduction_in_quantity(
        self, products_with_quantity: list[int, int], store_id
    ):
