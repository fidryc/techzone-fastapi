...
async def get_quantity_of_product(session: SessionDep, store_id: int, product_id: int) -> int:
    return await StoreQuantityInfoDao(session).get_quantity_of_product(store_id=store_id, product_id=product_id)