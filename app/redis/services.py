...
class RedisService:
    ...
    def validate_data(data):
        ...
        if not data:
            ...
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Нет ключа в redis"
            )
    ...
    def validate_ttl(ttl):
        ...
        if not ttl > 0:
            ...
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Время жизни вышло"
            )
    ...
    async def get_user_auth_data(self, key) -> UserAuthRedisSchema:
        ...
        try:
            ...
            logger.debug(
                "User auth data retrieved",
                extra={"key": key, "attempt": data["attempt"]},
            )
            return data
            ...
            logger.error(
                "Redis error getting user auth data", extra={"key": key}, exc_info=True
            )
            raise HTTPException(
            ...
            logger.error(
                "Unexpected error getting user auth data",
                extra={"key": key},
                exc_info=True,
            )
            raise HTTPException(
    ...
    async def set_user_auth_data(
        self, user_identifier: str, data: UserAuthRedisSchema
    ) -> None:
        ...
        try:
            ...
            logger.info(
                "User auth data saved to Redis",
                extra={
                    "user_identifier": user_identifier,
                    "ttl": settings.VER_CODE_EXP_SEC,
                },
            )
        except RedisError as e:
            logger.error(
                "Redis error saving user auth data",
                extra={"user_identifier": user_identifier},
                exc_info=True,
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            ...
            logger.error(
                "Unexpected error saving user auth data",
                extra={"user_identifier": user_identifier},
                exc_info=True,
            )
            raise HTTPException(
    ...
    async def get_dict(self, key: str) -> dict:
        ...
        try:
            ...
            logger.error(
                "Unexpected error getting dict from Redis",
                extra={"key": key},
                exc_info=True,
            )
            raise HTTPException(
    ...
    async def _validate_ip_limit(self, ip: str) -> None:
        ...
        try:
            ...
            logger.error(
                "Redis error validating IP limit", extra={"ip": ip}, exc_info=True
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def _set_limit_for_ip(self, ip: str):
        ...
        try:
            ...
            logger.info(
                "IP limit set", extra={"ip": ip, "ttl": settings.LIMIT_SECONDS_GET_CODE}
            )
        except RedisError as e:
            logger.error(
                "Redis error setting IP limit", extra={"ip": ip}, exc_info=True
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def validate_user_auth_data(
        self, data: UserAuthRedisSchema, code_from_user: int, redis_key: str
    ) -> None:
        """Валидирует данные для регистрации в redis"""
        try:
            if not self.is_correct_attempt(data["attempt"]):
                logger.warning(
                    "Max attempts exceeded", extra={"attempts": data["attempt"]}
                )
