...
class ProductServiceSync:
    def __init__(
        self, product_sync_dao: ProductSyncDao, review_sync_dao: ReviewSyncDao
    ):
    ...
    def update_reviews(self):
        ...
        try:
            ...
            logger.info(
                "Reviews updated successfully (sync)", extra={"count": len(avg_reviews)}
            )
...
class ProductService:
    def __init__(
        self, session: AsyncSession, product_dao: ProductDao, category_dao: CategoryDao
    ):
    ...
    def _validate_seller(user: UserSchema):
        ...
        if user.role != "seller":
            logger.warning(
                "Access denied: user is not a seller",
                extra={"user_id": user.user_id, "role": user.role},
            )
            raise HTTPException(
    ...
    async def _get_class_name(self, product: ProductSchema):
        ...
        try:
            category_row: Category = await self.category_dao.find_by_filter_one(
                category_id=product.category_id
            )
            if not category_row:
                logger.warning(
                    "Category not found", extra={"category_id": product.category_id}
                )
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND, detail="Категория не найдена"
                )
            logger.debug(
                "Category class name retrieved",
                extra={
                    "category_id": product.category_id,
                    "class_name": category_row.class_name,
                },
            )
            ...
            logger.error(
                "Failed to get category class name",
                extra={"category_id": product.category_id},
                exc_info=True,
            )
            raise HTTPException(
    ...
    def _validate_specification(self, product: ProductSchema, class_name):
        ...
        try:
            ...
            if not specification_schema:
                logger.warning(
                    "Specification schema not found", extra={"class_name": class_name}
                )
                raise HTTPException(
            ...
            logger.debug(
                "Specification validated successfully", extra={"class_name": class_name}
            )

        except ValidationError as e:
            logger.warning(
                "Specification validation failed",
                extra={"class_name": class_name, "errors": str(e.errors())},
            )
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            ...
            logger.error(
                "Unexpected error during specification validation",
    ...
    async def _add(self, product: ProductSchema):
        ...
        try:
            ...
            logger.info(
                ...
                extra={"title": product.title, "price": product.price},
            )
        except Exception as e:
            logger.error(
                "Failed to add product to database",
                extra={"title": product.title},
                exc_info=True,
            )
            raise

    async def add_product(
        self, product: ProductSchema, user: UserSchema, flag_notification: bool
    ):
        """
        ...
        try:
            ...
            if flag_notification:
                logger.info(
                    "Scheduling email notifications", extra={"title": product.title}
                )
                user_emails = await self.product_dao.get_user_emails_for_send_emails_about_new_product(
                    product
                )
                for email in user_emails:
                    send_email_about_new_product.delay(
                        email, title=product.title, price=product.price
                    )
                logger.info(
                    "Email notifications scheduled",
                    extra={"recipients_count": len(user_emails)},
                )
            ...
            logger.error(
                ...
                extra={"title": product.title, "user_id": user.user_id},
                exc_info=True,
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def recomendation(self, user_id):
        ...
        try:
            ...
            if not favorite_products:
                logger.info(
                    "User has no favorites, using popular products",
            ...
            if avg_price:
                logger.info(
                    "Using recommendations with avg price",
                    extra={"user_id": user_id, "avg_price": avg_price},
                )
                return await self.product_dao.get_recomentation_with_avg_price(
                    user_id, avg_price
                )
            else:
    ...
    async def get_product_by_id(self, product_id):
        ...
        try:
            ...
            query_update = (
                ...
                .values(views=product.views)
            )
            await self.session.execute(query_update)
            await self.session.commit()

            logger.info(
                "Product retrieved and views incremented",
                extra={"product_id": product_id, "views": product.views},
            )
            return product
...
class HistoryQueryTextService:
    ...
    async def add_history_query(self, user_id: int, query: str):
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail="Ошибка при добавлении запроса в таблицу с историей текстовых запросов",
            ) from e
        except Exception as e:
            logger.warning("Unknow error. Failed add the text quert to db table")
            raise HTTPException(
                status_code=500,
                detail="Неизвестная ошибка при добавлении текстового запроса в базу данных",
            ) from e
...
class SearchHistoryService:
    def __init__(
        ...
        el_service: ElasticsearchService,
        hqt_service: HistoryQueryTextService,
    ):
        self.session = session
    ...
    async def search_product(self, user_id, query: str) -> list[ProductSchema]:
        try:
            ...
            logger.warning(
                "Failed search products with add to history",
...
class CategoryService:
    ...
    async def get_example_spec_schema(self, category_id: int) -> dict:
        """Получение пример схемы для характеристик"""
        class_name = (
            await self.category_dao.find_by_filter_one(category_id=category_id)
        ).class_name
        return specification_schemas_dict[class_name].Settings.example
